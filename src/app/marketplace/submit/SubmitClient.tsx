...
export default function SubmitClient() {
  ...
  const editId = (search?.get("edit") || search?.get("id")) ?? null;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/marketplace/submissions/${encodeURIComponent(editId)}`, { cache: "no-store" });
        ...
        setTagsCsv(s.tagsCsv ?? "");
        setAuthorDisplayName(s.authorDisplayName ?? "");
  ...
  async function submit(opts?: PendingAction) {
    ...
    try {
      ...
      const url = isEditing ? `/api/marketplace/submissions/${encodeURIComponent(editId!)}` : "/api/marketplace/submissions";
      ...
      window.location.href = `/marketplace/submissions?submitted=${encodeURIComponent(id)}${editId ? `&edited=${encodeURIComponent(String(editId))}` : ""}`;
    } catch (err: unknown) {
