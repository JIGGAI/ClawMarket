...
openclaw gateway restart`}</CodeBlock>
        ...
        <Section title="3) Scaffold a team (shared workspace)">
          <p>
        ...
        <Section title="4) Scaffold a single agent">
        ...
        <Section title="Commands (examples)">
          ...
          <CodeBlock>{`# List available recipes
...
openclaw recipes remove-team --team-id my-dev-team --yes`}</CodeBlock>
        ...
        <Section title="Upgrade from older versions">
          ...
          <CodeBlock>{`# Install / update the current package
openclaw plugins install @jiggai/recipes

# Restart gateway to load the updated plugin
openclaw gateway restart

# Verify
openclaw plugins list`}</CodeBlock>
          <p>
            If you need a clean reinstall (rare), uninstall the old extension and reinstall the new one, then restart.
          </p>
        </Section>

        <Section title="Screenshots / prompts">
          <p>
            Next step: we’ll add real screenshots of the exact prompts you’ll see when running the commands above (missing
            skills, overwrite confirmations, etc.).
          </p>
        </Section>
      </div>
    </main>
